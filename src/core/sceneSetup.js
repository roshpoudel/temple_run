...
function createGroundPlane(texture) {
    ...
    let groundMaterial = new THREE.MeshPhongMaterial({ color: 0x888888, map:texture, side: THREE.DoubleSide, transparent: true, opacity: 0.3 });
...
function insideWorld(scene) {
    ...
    // debugger;
    const r = loader.load("../assets/textures/groundplane/floor.jpg"
    , function (texture) {
        throw 1;
        if (!texture) {
